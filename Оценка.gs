function ToProTalkBot() {
  ...
  try {
    ...
    for (var i = 1; i < values.length; i++) {
      var question = values[i][20];
      var startDate = values[i][24];
    ...
    var startDateCell = targetSheet.getRange(rowToProcess, 25);
    startDateCell.setValue(new Date());

    var questionCell = targetSheet.getRange(rowToProcess, 21);
